...
(function($)
{
	...
	$.dateRangePickerLanguages =
	{
		'en':
		{
			...
			'default-more' : 'Select a date range longer than %d days',
			'default-single' : 'Select a date',
			'default-less' : 'Select a date range less than %d days',
			'default-range' : 'Select a date range between %d and %d days',
			'default-default': 'Select a date range'
		}
	};
      ...
      return r;
		...
		function createDom()
		{
			...
			html += '</div>\
					...
					<input type="button" class="btn apply-btn disabled'+ getHideClass() +'" value="'+lang('apply')+'" />\
				</div>'
			...
			if ( ! opt.singleDate ) {
				html += '<table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>'
			}
				//+'</div>'
			...
			if (opt.showShortcuts)
			{
				...
				{
                    html += '<span><b>&nbsp;| Filter:</b></span>'
					for(var i=0;i<opt.customShortcuts.length; i++)
					{
						var sh = opt.customShortcuts[i];
                        if(sh.name === 'Default'){
                            html+= '&nbsp;<span class="custom-shortcut"><input id="'+i+'" type="radio" name="shortcut" shortcut="custom" value="'+ sh.name +'" checked> '+sh.name+'</span>';
                        }else{
                            html+= '&nbsp;<span class="custom-shortcut"><input id="'+i+'" type="radio" name="shortcut" shortcut="custom" value="'+ sh.name +'"> '+sh.name+'</span>';
                        }
					}
				}
